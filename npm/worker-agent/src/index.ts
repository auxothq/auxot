...
function detectPlatform(): { os: string; arch: string } {
  ...
  if (!mappedOS) {
    ...
    console.error("auxot-agent supports Linux and macOS.");
    process.exit(1);
  }
  if (!mappedArch) {
    console.error(`Unsupported architecture: ${cpuArch}`);
    console.error("auxot-agent supports x64 (amd64) and arm64.");
    process.exit(1);
  }
...
async function downloadBinary(
  ...
  console.log(
    `Downloading auxot-agent ${version} for ${plat.os}/${plat.arch}...`
  );
...
async function main() {
  ...
  if (
    ...
    console.log(`@auxot/worker-agent v${pkg.version} (Go binary wrapper)`);
    process.exit(0);
  }

  if (args.includes("--help") || args.includes("-h") || args[0] === "help") {
    console.log(`@auxot/worker-agent — Run an Auxot agent worker

This package downloads and runs the auxot-agent Go binary.
All arguments are passed through to the binary.

Usage:
  npx @auxot/worker-agent --agent-key agnt_xxx --router-url wss://your-server/ws

Options:
  --agent-key <key>     Agent key from Auxot settings (required)
  --router-url <url>    Router URL (required)
  --dir <path>          Gitagent directory (default: current directory)
  --log-level <level>   Log level: debug, info, warn, error
  --help                Show this help
  --version             Show version

Environment Variables:
  AUXOT_AGENT_KEY       Agent key (alternative to --agent-key)
  AUXOT_ROUTER_URL      Router URL (alternative to --router-url)
  AUXOT_AGENT_DIR       Gitagent directory (alternative to --dir)
...
More info: https://github.com/auxothq/auxot`);
  ...
  try {
    ...
    console.error("Failed to download auxot-agent:", err.message);
    process.exit(1);
  }
